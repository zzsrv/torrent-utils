...
import cc.vcode.torrent.TOTorrentAnnounceURLGroup;
...
import cc.vcode.util.BEncoder;
import cc.vcode.util.ByteFormatter;
import cc.vcode.util.Constants;
import cc.vcode.util.HashWrapper;
import cc.vcode.util.NonDaemonTask;
import cc.vcode.util.NonDaemonTaskRunner;
import cc.vcode.util.SHA1Hasher;
...
public class TOTorrentImpl implements TOTorrent
{
	...
	protected static final String TK_PUBLISHER = "publisher";

	protected static final String TK_PUBLISHER_URL = "publisher-url";

	private byte[] torrent_name;

	private byte[] torrent_publisher;

	private byte[] torrent_publisher_url;
	...
	private Map<String, Object> additional_info_properties = new HashMap<String, Object>();
	...
	protected TOTorrentImpl(String _torrent_name, URL _announce_url, boolean _simple_torrent) throws TOTorrentException
	{
		...
		} catch (UnsupportedEncodingException e)
		{
			...
			throw (new TOTorrentException("TOTorrent: unsupported encoding for '" + _torrent_name + "'", TOTorrentException.RT_UNSUPPORTED_ENCODING));
		}
	}



	public void serialiseToBEncodedFile(final File output_file) throws TOTorrentException
	{
		try
		{
			NonDaemonTaskRunner.run(new NonDaemonTask()
			{
				public Object run() throws Throwable
				{
					...
					} catch (Throwable e)
					{
						...
						throw (new TOTorrentException("TOTorrent::serialise: fails '" + e.toString() + "'", TOTorrentException.RT_WRITE_FAILS));
	...
	protected byte[] serialiseToByteArray() throws TOTorrentException
	{
		...
		} catch (IOException e)
		{
			...
			throw (new TOTorrentException("TOTorrent::serialiseToByteArray: fails '" + e.toString() + "'", TOTorrentException.RT_WRITE_FAILS));
	...
	public Map<String, Object> serialiseToMap() throws TOTorrentException
	{
		...
		if (sets.length > 0)
		{
			...
			List<List<Object>> announce_list = new ArrayList<List<Object>>();
			...
			for (int i = 0; i < sets.length; i++)
			{
				...
				List<Object> sub_list = new ArrayList<Object>();
		...
		// 添加
		info.put(TK_PUBLISHER, torrent_publisher);

		info.put(TK_PUBLISHER_URL, torrent_publisher_url);
	...
	protected String readStringFromMetaData(byte[] value) throws TOTorrentException
	{
		...
		} catch (UnsupportedEncodingException e)
		{
			...
			throw (new TOTorrentException("TOTorrentDeserialise: unsupported encoding for '" + value + "'", TOTorrentException.RT_UNSUPPORTED_ENCODING));
		}
	}



	protected void writeStringToMetaData(Map<String, Object> root, String name, String value) throws TOTorrentException
	{
		root.put(name, writeStringToMetaData(value));
	}
	...
	protected byte[] writeStringToMetaData(String value) throws TOTorrentException
	{
		...
		} catch (UnsupportedEncodingException e)
		{
			...
			throw (new TOTorrentException("TOTorrent::writeStringToMetaData: unsupported encoding for '" + value + "'", TOTorrentException.RT_UNSUPPORTED_ENCODING));
		}
	}
	...
	public void print() throws Exception
	{
		try
		{
			...
			System.out.println("name = " + new String(torrent_name, Constants.DEFAULT_ENCODING));
			...
			System.out.println("creation by = " + new String(created_by, Constants.DEFAULT_ENCODING));
			System.out.println("comment = " + new String(comment, Constants.DEFAULT_ENCODING));
			...
			System.out.println("publisher = " + new String(getPublisher(), Constants.DEFAULT_ENCODING));
			System.out.println("publisher-url = " + new String(getPublisherUrl(), Constants.DEFAULT_ENCODING));
	...
	public void setName(String _name)
	{
		try
		{
			...
			setName(_name.getBytes(Constants.DEFAULT_ENCODING));
	...
	{
		return torrent_publisher;
	}



	public void setPublisher(byte[] _publisher)
	{
		torrent_publisher = _publisher;
	}
	...
	public void setPublisher(String _publisher)
	{
		try
		{
			...
			setPublisher(_publisher.getBytes(Constants.DEFAULT_ENCODING));
		...
		} catch (UnsupportedEncodingException e)
		{
			...
			torrent_publisher = null;
		}
	}



	public byte[] getPublisherUrl()
	{
		return torrent_publisher_url;
	}



	public void setPublisherUrl(byte[] _publisher_url)
	{
		torrent_publisher_url = _publisher_url;
	}



	public void setPublisherUrl(String _publisher_url)
	{
		try
		{

			setPublisherUrl(_publisher_url.getBytes(Constants.DEFAULT_ENCODING));
	...
	protected void setHashFromInfo(Map<?, ?> info) throws TOTorrentException
	{
		...
		} catch (Throwable e)
		{
			...
			throw (new TOTorrentException("TOTorrent::setHashFromInfo: fails '" + e.toString() + "'", TOTorrentException.RT_HASH_FAILS));
		}
	}
	...
	protected void setPieces(byte[][] _pieces)
	{
		pieces = _pieces;
	}
